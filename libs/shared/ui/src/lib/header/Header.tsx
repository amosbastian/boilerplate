...
import { RiBook3Fill, RiGroupFill, RiPhoneFill, RiQuestionAnswerFill } from "react-icons/ri";
...
const HEADER_LINKS = [
  {
    id: "features",
    links: [
      { href: "#", id: "feature-1", icon: undefined },
      { href: "#", id: "feature-2", icon: undefined },
    ],
  },
  { id: "pricing", links: [{ href: "/pricing", id: "pricing", icon: undefined }] },
  {
    id: "resources",
    links: [
      ...
      <Container display="flex" justifyContent="space-between" alignItems="center" h={16} w="100%">
        ...
        <HStack alignItems="baseline" spacing={6}>
          {HEADER_LINKS.map((linkItem) => {
            if (linkItem.links.length > 1) {
              return (
                <DropdownMenu label={t(linkItem.id)}>
            ...
            return (
              <HeaderLink key={linkItem.id} href={linkItem.links[0].href}>
                {t(linkItem.links[0].id)}
              </HeaderLink>
            );
          })}
        </HStack>
